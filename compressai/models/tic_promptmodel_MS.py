...
class TIC_PromptModelMS(nn.Module):
    """
    ...
    def g_a(self, x, mask, x_size=None):
        attns = []
        if x_size is None:
            x_size = x.shape[2:4]
    ...
    def forward(self, x, lamb=None, decoder_lamb=None, lidar=None):
        x_size = (x.shape[2], x.shape[3])
        y, attns_a = self.g_a(x, lamb, x_size)
        z = self.h_a(y, x_size)
        ...
        x_hat, attns_s = self.g_s_lidar(y_hat, x_size, decoder_lamb, lidar)
...
net = TIC_PromptModelMS(N=128, M=192, prompt_config=pc, input_resolution=(256,256))
"""
...
lmd = torch.zeros(1, 1, 256, 256, dtype=torch.float, requires_grad=False)
lmd_decoder = torch.zeros(1, 1, 16, 16, dtype=torch.float, requires_grad=False)
lidar = torch.zeros(1, 1, 256, 256, dtype=torch.float, requires_grad=False)

net.forward(x, lmd, lmd_decoder, lidar)
"""
